...
import {
  ASSISTANT_NAME,
  DATA_DIR,
  FEISHU_ASSISTANT_NAME,
  FEISHU_TRIGGER_PATTERN,
  IDLE_TIMEOUT,
  MAIN_GROUP_FOLDER,
  POLL_INTERVAL,
  TRIGGER_PATTERN,
} from './config.js';
import { FeishuChannel } from './channels/feishu-sdk-ws.js';
import {
  ContainerOutput,
  runContainerAgent,
  writeGroupsSnapshot,
...
async function processGroupMessages(chatJid: string): Promise<boolean> {
  ...
  await feishu.setTyping?.(chatJid, true);
  let hadError = false;
  ...
  const output = await runAgent(group, prompt, chatJid, async (result) => {
    ...
    if (result.result) {
      ...
      if (text) {
        await feishu.sendMessage(chatJid, `${FEISHU_ASSISTANT_NAME}: ${text}`);
  ...
  await feishu.setTyping?.(chatJid, false);
  if (idleTimer) clearTimeout(idleTimer);
...
async function startMessageLoop(): Promise<void> {
  ...
  logger.info(`NanoClaw running with Feishu (trigger: @${FEISHU_ASSISTANT_NAME})`);
...
function cleanupOrphanedContainers(): void {
  ...
  try {
    ...
    const containers: { Names?: string; State?: string }[] = lines.map(l => JSON.parse(l));
    ...
    // Fall through to Apple Container
  }

  // Try Apple Container (macOS)
  try {
    ...
    if (orphans.length > 0) {
      logger.info({ count: orphans.length, names: orphans }, 'Stopped orphaned Apple containers');
    }
  } catch (err) {
...
async function main(): Promise<void> {
  ...
  startSchedulerLoop({
    ...
    sendMessage: async (jid, rawText) => {
      const text = formatOutbound(feishu, rawText);
      if (text) await feishu.sendMessage(jid, text);
    },
  });
  startIpcWatcher({
    sendMessage: (jid, text) => feishu.sendMessage(jid, text),
    registeredGroups: () => registeredGroups,
    registerGroup,
    syncGroupMetadata: (force) => feishu.syncGroupMetadata(force),
    getAvailableGroups,
